...
\begin{figure}[h!]
	...
	\caption{Few samples of graphemes in Malayalam-ADD PHONETIC EQUIVALENT HERE}
...
\paragraph{}
The first native type casting and printing was done by Benjamin Bailey, an Anglican missionary in 1829\cite{babucherian}.  His contributions as a typographer made the curvy style of the Malyalam orthography popular\cite{gupthannair}. Figure \ref{newtestament}, shows pages from The New Testament printed using the types designed by Benjamin Bailey, imprinted in 1829\cite{babucherian}. The script continued to evolve by separating some vowel sign types ({\manjari{  ി, ീ } }) from the consonant/conjunct grapheme. Still the richness of conjuncts and their signed forms were largely retained. It can be seen from Figure \ref{Sabdatharavali}. 
...
As per the governement order the reformed orthography would contain only90 unique graphemes.
\end{itemize}
